...
function autocomplete(input, latInput, lngInput) {
    if (!input) return; //skip if fn from running if there is not input on the page
    ...
    searchInput.on('input', function () {
        ...
        _axios2.default.get('/api/search/?q=' + this.value).then(function (res) {
            ...
            if (res.data.length) {
                ...
                searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results found for <strong>' + _this.value + '</strong> found!.</div>');
            }
        }).catch(function (err) {
            consol.error(err);
        });
